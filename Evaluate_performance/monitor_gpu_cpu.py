...
def check_gpu_mem_usedRate(share_sign,share_log):
    ...
    while True:
        ...
        if not share_sign['state']:
            break # 跳出监控gpu循环
    share_log['cpu-apply-area']=points_lst
    share_log['gpu-used-ratio'] = linear
    share_sign['close'] = True

def check_cpu_mem_usedRate(share_sign,share_log):
    linear = []
    points=[]
    points_lst=[]
    n=0
    while True:
        info = get_device_info()
        linear.append(psutil.virtual_memory().percent)
        if share_sign['recording'][1] and len(points)==0:
            points=[n]
        elif (n!=0) and (not share_sign['recording'][1]) and len(points)==1:
            points += [n]
        ...
        if not share_sign['state']:
            break # 跳出监控gpu循环
    share_log['cpu-apply-area'] = points_lst
    share_log['cpu-used-ratio'] = linear
    while True:
        time.sleep(0.5)
        if share_sign['close']:
            break
...
def draw_fig(share_log,figpth):
    ...
    dy = share_log['gpu-used-ratio']
    dx = np.linspace(0,len(dy),len(dy))
    plt.plot(dx,dy,colors_plot[0],label='gpu-used-ratio',marker='.')
    hy = share_log['cpu-used-ratio']
    hx = np.linspace(0,len(hy),len(hy))
    plt.plot(hx,hy,colors_plot[1],label='gpu-used-ratio',marker='.')

    start = 0.0
    stop = 1.0
    number_of_lines= (share_log['gpu-apply-area'])*2
    cm_subsection = np.linspace(start, stop, number_of_lines) 
    colors_pan = [ cm.jet(x) for x in cm_subsection ]
     
    for i,dpoints in enumerate(share_log['gpu-apply-area']):
        dvalues = list(dpoints.values())
        plt.axvspan(xmin=dvalues[0][0], xmax=dvalues[0][1], facecolor=colors_pan[i], alpha=0.3,label=list(dpoints.keys())[0])  

    for j,hpoints in enumerate(share_log['cpu-apply-area']):
        hvalues = list(hpoints.values())
        plt.axvspan(xmin=hvalues[0][0], xmax=hvalues[0][1], facecolor=colors_pan[j+number_of_lines], alpha=0.1,label=list(hpoints.keys())[0])  
